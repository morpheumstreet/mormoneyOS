...
4. **DRY / CLEAN / SOLID:** Single registration path, split responsibilities, shared base for command-based providers.
...
### 6.2 RegistryOptions Extension (Single Path — DRY)
...
### 6.3 Split Responsibilities (SOLID S, I)
...
### 6.4 CommandTunnelProvider Base (DRY)
...
bore, localtunnel, custom extend this; cloudflare/ngrok add token handling. **DRY:** subprocess spawn + URL extraction live in one place.
...
## 7. Construction & Bootstrap (CLEAN)
...
## 12. DRY / CLEAN / SOLID Summary

| Principle | Application |
|-----------|-------------|
| **DRY** | Single `ServiceProvider` registration path; `CommandTunnelProvider` base for bore/localtunnel/custom; shared subprocess + URL extraction logic. |
| **CLEAN** | Tools depend on `TunnelManager` interface; config → construction in `NewFromConfig`; secrets from env/config. |
| **S**ingle Responsibility | `ProviderRegistry` = register/list; `TunnelManager` = start/stop/status; `CommandTunnelProvider` = subprocess + URL parse. |
| **O**pen/Closed | New providers via `Register()`; new services via `ServiceProviders`; no core edits. |
| **I**nterface Segregation | `TunnelProvider` minimal; `TunnelManager` separate from `ProviderRegistry`; tools need only `TunnelManager`. |
| **D**ependency Inversion | Tools → `TunnelManager`; Manager → `ProviderRegistry`; all via interfaces. |

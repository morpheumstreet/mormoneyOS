...
| **Schema**            | v8, 22+ tables                                                                                       | v13, 26 tables (includes 5-tier memory: working_memory, episodic_memory, semantic_memory, procedural_memory, relationship_memory) | ⚠️ Go subset; soul reflection + metrics + 5-tier aligned |
...
cmd/moneyclaw/main.go → cmd.Execute()
  └── run
        ├── config.Load()
        ├── state.Open() — SQLite, SchemaV1 + migrations (v13: 5-tier memory; v12: metric_snapshots, etc.)
...
| working_memory         | ✅   | ✅   | TS-aligned; session_id, content, content_type, priority, expires_at (schema v13) |
| episodic_memory        | ✅   | ✅   | TS-aligned; session_id, summary, outcome, importance (schema v13)                  |
| semantic_memory        | ✅   | ✅   | TS-aligned; category, key, value, confidence (schema v13)                           |
| procedural_memory      | ✅   | ✅   | TS-aligned; name, steps, success_count, failure_count (schema v13)                 |
| relationship_memory    | ✅   | ✅   | TS-aligned; entity_address, trust_score, interaction_count (schema v13)            |
...
**Go has:** inference_costs, heartbeat_dedup, skills, children, transactions, inbox_messages (v11), metric_snapshots (v12), **5-tier memory** (v13): working_memory, episodic_memory, semantic_memory, procedural_memory, relationship_memory.

**Schema gap summary:** Soul reflection evidence (transactions, inbox_messages); metric_snapshots (report_metrics) in v12; 5-tier memory tables in v13. Remaining TS-only: soul_history, model_registry, modifications, reputation, session_summaries, discovered_agents_cache.
...
| 5. Build system prompt     | `buildSystemPrompt`                                                 | `BuildSystemPrompt`                                                                                                                                                                             | ✅ TS-aligned: status, credits, tier, genesis, lineageSummary; skills via Subconscious + FormatForPrompt when SkillsConfig set |
| 6. Retrieve memories       | `MemoryRetriever.retrieve`, `formatMemoryBlock`                     | `memory.DBMemoryRetriever` (DB-only, 5-tier tables); BudgetAllocator; inject at index 1                                                                                   | ✅ Aligned (Phase 3: DB-only retrieval)                                                         |
...
**Loop wiring (`cmd/run.go`):** `NewLoopWithOptions` with `Store` (db), `Inference` (OpenAIClient when keys set), `Tools` (Registry), `Config`, `CreditsFn` (Conway when configured), `LineageStore` (db for GetLineageSummary), `MemoryRetriever` (DBMemoryRetriever with db, budget).

**Summary:** Core ReAct path (steps 1–13) is aligned. Step 13 (loop/idle/sleep): TurnResult, IsMutatingTool, sleep tool immediate transition, finishReason stop — see [loop-step13-implementation.md](./loop-step13-implementation.md). Inbox claim (2) implemented: ClaimInboxMessages, MarkInboxProcessed, InsertInboxMessage; check_social_inbox persists to inbox_messages. Survival tier (4): conway.TierFromCreditsCents, tierToAgentState, TierStateStore.SetAgentState, inference.SetLowComputeMode, LowComputeModel selection. Memory retrieval (6): DBMemoryRetriever (DB-only, 5-tier: working, episodic, semantic, procedural, relationship); BudgetAllocator; inject at index 1 (TS-aligned). 5-tier tables in schema v13.
...
| Memory (5-tier)  | ✅                      | ✅                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Go: 5-tier tables (schema v13); DBMemoryRetriever (DB-only); BudgetAllocator; wired in cmd/run.go                                                                                                                                                                                                              |
...
- ~~**Skills prompt injection:** Subconscious + FormatForPrompt~~ ✅ **Done:** `internal/skills/loader.go` (LoadAllFromStore, SKILL.md/SKILL.toml merge); `BuildSystemPrompt` injects via `FormatForPrompt` when SkillsConfig set.
- ~~Soul system: view_soul, update_soul, reflect_on_soul, view_soul_history~~ ✅
...
- ~~**5-tier memory tables + retrieval**~~ ✅ **Done:** schema v13 (working_memory, episodic_memory, semantic_memory, procedural_memory, relationship_memory); DBMemoryRetriever (DB-only); BudgetAllocator; wired in cmd/run.go.
...
- [memory-system-5-tier.md](./memory-system-5-tier.md) — 5-tier memory design: tiers, why each matters, Go Phase 1 vs Phase 2 gap.
- [memory-retrieval-step6.md](./memory-retrieval-step6.md) — Memory retrieval interface, KV-backed Phase 1, loop integration.
...
- Skills prompt injection (Subconscious, SKILL.md/SKILL.toml merge, FormatForPrompt)
- Soul system (view/update/reflect/history)
- 5-tier memory (working_memory, episodic_memory, semantic_memory, procedural_memory, relationship_memory tables; DBMemoryRetriever DB-only; BudgetAllocator); tools still write to KV until migration
